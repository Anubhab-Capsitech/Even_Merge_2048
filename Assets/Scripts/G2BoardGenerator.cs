...
public class G2BoardGenerator : MonoBehaviour
{
	...
	private void Start()
	{
		...
		GameList.Instance.UiSwitching(true);
		GameList.Instance.BarAnimation(true);
		Debug.Log("game start UiSwitching(false)");
		PlayerAnalytics.Instance?.StartSession();  // firebase analytics - session start on game start
	...
	public void AutoDelete()
	{
		...
		PlayerAnalytics.Instance.RegisterChainReaction(); // firebase analytics - chain reaction
	}
	...
	{
		// Only load saved blood data if there's actually valid saved data
		// Check both isSavedGame() AND that the data exists (wasn't deleted after game over)
		if (GM.GetInstance().isSavedGame() && this.IsFinishGuide && PlayerPrefs.HasKey("LocalData_OldLife"))
		{
			this.InitBlood();
			this.LoadLocalLife();
			return;
	...
	{
		this.InitMap();
		// Only load saved map data if there's actually valid saved data
		// Check both isSavedGame() AND that the data exists (wasn't deleted after game over)
		if (GM.GetInstance().isSavedGame() && this.IsFinishGuide && PlayerPrefs.HasKey("LocalData_OldGame"))
		{
			this.LoadLocalData();
			return;
		}
	...
	private void Delete(List<int> list, int index)
	{
		...
		foreach (int current in list)
		{
			...
			if (current != index)
			{
                ...
                // Removed AddExp call - XP is now calculated from High Score in GM
	...
	private void AddScore(int number)
	{
		...
		Debug.Log($"[AddScore] Score={Score}, MaxScore={MaxScore}");
	}
