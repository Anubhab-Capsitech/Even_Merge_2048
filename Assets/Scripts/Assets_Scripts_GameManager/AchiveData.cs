...
namespace Assets.Scripts.GameManager
{
	internal class AchiveData
	{
		...
		public void Add(int type, int value, bool isMerge = true)
		{
			...
			// In some scenes (like the 3D gameplay scene) there may be no DotManager
			// instance at all, or the UI object may have been destroyed when switching
			// scenes. Only try to refresh the dots if a valid instance exists.
			DotManager dotManager = DotManager.GetInstance();
			if (dotManager != null)
			{
				dotManager.CheckAchiev();
			}
		...
		public bool Finish(int type)
		{
			...
			this.achives[type.ToString()] = localData;
			this.SetLocalData(localData.type, localData.value, localData.key, localData.status);
			// See comment in Add(): guard against missing/destroyed DotManager
			DotManager dotManager = DotManager.GetInstance();
			if (dotManager != null)
			{
				dotManager.CheckAchiev();
		...
		private void Read()
		{
			// Clear the dictionary if it already has data (prevents duplicate key errors when scene reloads)
			if (this.achives.Count > 0)
			{
				this.achives.Clear();
			}
			...
			foreach (TAchive current2 in list)
			{
				...
				// Use indexer instead of Add to avoid duplicate key errors (will overwrite if exists)
				this.achives[current2.Type.ToString()] = localData;
			}
		}
